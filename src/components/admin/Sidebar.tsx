...
export default function AdminSidebar() {
  ...
  return (
    <aside className="w-60 bg-stone-900 border-r border-stone-800 flex flex-col">
      {/* Logo */}
      <div className="px-5 py-5 border-b border-stone-800">
        <h2 className="text-lg font-bold text-stone-50 tracking-tight">
          Social<span className="text-orange-500">Shares</span>
        </h2>
        <p className="text-xs text-stone-400 uppercase tracking-[0.15em] mt-0.5">
          Admin Panel
        </p>
      </div>
      ...
      <nav className="flex-1 p-3 space-y-1">
        {navItems.map((item) => {
          ...
          return (
            ...
            >
              <span className={isActive ? "text-orange-500" : "text-stone-500"}>
                {item.icon}
              </span>
              {item.label}
            </Link>
      ...
      <div className="p-3 border-t border-stone-800">
        <button
          ...
          className="flex items-center gap-3 w-full px-3 py-2.5 text-sm text-stone-500 hover:text-red-400 hover:bg-red-500/5 rounded-xl transition-all duration-150 text-left"
        >
          <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
